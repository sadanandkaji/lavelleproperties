...
const FurnishingContent = () => {
  ...
  return (
    <div className="relative z-10 w-full max-w-[600px] text-center">
      ...
      {/* Back Button - Pushed further down with top-40 */}
      <button 
        onClick={() => router.push(`/layouttype?type=${propertyType}&subtype=${subType}`)}
        className="fixed left-6 top-32 z-50 flex h-12 w-12 items-center justify-center rounded-full border border-[#d4af3744] bg-white/10 backdrop-blur-md text-[#c5a059] shadow-lg transition-all duration-300 hover:scale-110 hover:bg-[#d4af37] hover:text-white hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] active:scale-95 max-md:left-4 max-md:top-32 max-md:h-10 max-md:w-10"
      ...
      {/* Main heading pushed even further down to clear the new back button position */}
      ...
      <div className="flex w-full flex-col items-center gap-[35px] max-md:gap-[25px]">
        {furnishingOptions.map((option) => (
          <Link 
            key={option.value} 
            href={`/amenities?type=${propertyType}&subtype=${subType}&layouttype=${layoutType}&furnishing=${option.value}`} 
            className="flex w-[280px] justify-center no-underline"
          >
            <button className="group relative flex h-[70px] w-[240px] items-center justify-center rounded-full bg-gradient-to-b from-[#d4af37] to-[#b8860b] shadow-[0_10px_0_#8b6d1d,0_20px_40px_rgba(0,0,0,0.15)] transition-all duration-200 hover:-translate-y-[3px] active:translate-y-[6px] max-md:h-[65px] max-md:w-[220px]">
...
const FurnishingPage: React.FC = () => {
  return (
    /* Increased top padding (pt-52) to keep everything visually low and clear of the navbar */
