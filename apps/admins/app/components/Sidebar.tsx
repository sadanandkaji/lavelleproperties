    ...
    <aside style={{
      ...
      <div style={{ padding: '22px 20px 16px', borderBottom: '1px solid #1a1a1a' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 800, color: '#fff', letterSpacing: '0.14em', textTransform: 'uppercase' }}>
            LAVELLE
          </span>
        </div>
        <div style={{ fontSize: 9, color: '#3a3a3a', letterSpacing: '0.22em', marginTop: 5, textTransform: 'uppercase' }}>
          Admin Panel
        </div>
      </div>
      ...
      <nav style={{ flex: 1, padding: '12px 8px', overflowY: 'auto' }}>
        {NAV.map((item, index) => {
          ...
          if ('top' in item && item.top === true) {
            ...
            return (
              <Link key={topItem.href} href={topItem.href} style={{ textDecoration: 'none', display: 'block', marginBottom: 8 }}>
                <div style={{
                  display: 'flex', alignItems: 'center', gap: 9,
                  padding: '9px 12px', borderRadius: 8,
                  background: active ? '#10b98118' : '#111',
                  border: `1px solid ${active ? '#10b98135' : '#1e1e1e'}`,
                  color: active ? '#10b981' : '#888',
                  cursor: 'pointer', transition: 'all 0.15s',
                }}>
                  <span style={{ color: active ? '#10b981' : '#444', flexShrink: 0 }}>{topItem.icon}</span>
                  <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                    {topItem.label}
                  </span>
                  {active && (
                    <span style={{ marginLeft: 'auto', width: 4, height: 4, borderRadius: '50%', background: '#10b981' }} />
                  )}
                </div>
              </Link>
            )
            ...
            <div key={section.label} style={{ marginBottom: 2 }}>
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer', transition: 'all 0.15s',
                  color: isOpen ? '#d4d4d4' : '#555',
                }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ color: isOpen ? '#10b981' : '#333', flexShrink: 0 }}>
                    {section.icon}
                  </span>
              ...
              {isOpen && (
                <div style={{ marginLeft: 12, paddingLeft: 10, borderLeft: '1px solid #1e1e1e', marginBottom: 6 }}>
                    ...
                    return (
                      <Link key={child.href} href={child.href} style={{ textDecoration: 'none', display: 'block' }}>
                        <div style={{
                          ...
                          <span style={{ fontSize: 11, fontWeight: active ? 700 : 400, letterSpacing: '0.05em', flex: 1 }}>
                            {child.label}
                          </span>
                          {active && (
        ...
        { (
          <button
            ...
            style={{
              ...
              color: '#ef4444', cursor: 'pointer', transition: 'all 0.15s', textAlign: 'left',
            }}
            onMouseEnter={e => {
