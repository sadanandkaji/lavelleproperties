...
export default function TemplateCreator({ onSave, onCancel }: { onSave?: (data: any) => void; onCancel?: () => void }) {
  ...
  // Cloudinary result for uploaded media
  const [mediaUrl, setMediaUrl]         = useState('')      // public CDN URL (preview + WhatsApp)
  const [mediaPublicId, setMediaPublicId] = useState('')
  const [mediaLocalPreview, setMediaLocalPreview] = useState('') // blob URL for instant preview
...
const [metaHandle, setMetaHandle] = useState<string>('') // Add this with your other state variables
...
// In TemplateCreator component - update handleFileChange
const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
  const file = e.target.files?.[0]
  if (!file) return

  // Instant local preview (blob URL)
  ...
  try {
    // Upload to Meta (required for WhatsApp template)
    ...
    if (!metaData.success || !metaData.handle) {
      throw new Error(metaData.error || 'Meta upload failed')
    }

    const metaHandleValue = metaData.handle
    setMetaHandle(metaHandleValue)
    console.log('✅ Meta upload complete - Handle:', metaHandleValue)

    // Upload to Cloudinary (for storage/preview in your DB)
    const cloudForm = new FormData()
    cloudForm.append('file', file)

    const cloudRes = await fetch('/api/upload1', { 
      method: 'POST', 
      body: cloudForm 
    })
    ...
    if (cloudData.success) {
      ...
      console.log('✅ Cloudinary upload complete - URL:', cloudinaryUrl)
    } else {
      console.warn('Cloudinary upload failed:', cloudData.error)
    }
...
const validate = () => {
  ...
  // Check for metaHandle (not mediaUrl)
 ...
 // In TemplateCreator component - fix the handleSave function
const handleSave = async () => {
  if (!validate()) return
  
  // Check if media header requires a Meta handle
  if (['image', 'video', 'document'].includes(headerType) && !metaHandle) {
    setModal({
      type: 'error',
      title: '✕ Media not uploaded to Meta',
      message: 'Please upload your media file first.',
    })
    return
  }
  ...
  try {
    const res = await fetch('/api/templates', {
      ...
      body: JSON.stringify({
        ...
        headerHandle: metaHandle,           // Meta handle for WhatsApp
        headerMediaUrl: mediaUrl,           // Cloudinary URL for your DB
    ...
    if (data.success) {
      setModal({
        ...
        message: data.message ?? 'Template submitted to Meta for review.',
      })
      onSave?.(data.data)
    } else {
      // Error handling...
...
setMetaHandle('')
  ...
  // Preview image: local blob (instant) → Cloudinary URL → placeholder
  ...
  setMetaHandle('')
          ...
          {headerType === 'text' && (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                {errors.headerText ? <span style={{ fontSize: 11, color: '#ef4444' }}>{errors.headerText}</span>
  ...
  <div>
    ...
    <div onClick={() => fileRef.current?.click()}
      style={{
        ...
        borderRadius: 10, padding: mediaLocalPreview && headerType === 'image' ? 0 : '24px',
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
              <div>
                {metaHandle && (
                  <div style={{ fontSize: 10, color: '#00e5a0' }}>✓ Meta: Ready</div>
                )}
                {mediaUrl && (
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.6)' }}>✓ Cloudinary: Saved</div>
                )}
              </div>
    ...
    {metaHandle && !uploading && (
      <div style={{ marginTop: 8, fontSize: 11, color: 'var(--muted)', wordBreak: 'break-all' }}>
        ...
        <div style={S.section}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            {errors.body ? <span style={{ fontSize: 11, color: '#ef4444' }}>{errors.body}</span>
        ...
        <div style={S.section}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            {errors.footer ? <span style={{ fontSize: 11, color: '#ef4444' }}>{errors.footer}</span>
        ...
        <div style={S.section}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 12 }}>
            {buttons.map((btn, i) => (
              <div key={i} style={{ background: 'var(--surface2)', border: `1px solid ${errors[`btn_${i}`] ? '#ef4444' : 'var(--border)'}`, borderRadius: 10, padding: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          {onCancel && (
            <button onClick={onCancel} style={{ padding: '12px 24px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--muted)', fontSize: 13, fontFamily: 'var(--font-mono)', cursor: 'pointer' }}>
              Cancel
            </button>
          )}
          <button onClick={handleSave} disabled={saving || uploading} style={{
            padding: '12px 28px', borderRadius: 8, border: '1px solid #00e5a050',
            background: '#00e5a015', color: '#00e5a0', fontSize: 13,
      ...
      <div style={{ position: 'sticky', top: 24 }}>
        ...
        <div style={{ background: '#1a1a2e', borderRadius: 24, padding: '18px 10px', border: '6px solid #333' }}>
          ...
          <div style={{ background: '#0b1418', borderRadius: 14, minHeight: 280, padding: 10,
            ...
            <div style={{ maxWidth: '85%', background: '#202c33', borderRadius: '0 12px 12px 12px', overflow: 'hidden', boxShadow: '0 2px 8px #0004' }}>
              ...
              {buttons.length > 0 && (
                <div style={{ borderTop: '1px solid #3b4a54' }}>
                  ...
                  {buttons.length > 3 && <div style={{ padding: '9px 14px', textAlign: 'center', fontSize: 12, color: '#53bdeb', borderTop: '1px solid #3b4a54' }}>≡ See all {buttons.length} options</div>}
                </div>
              )}
            </div>
          </div>
...
{ 
  ok: !['image','video','document'].includes(headerType) || !!metaHandle,
  text: 'Media uploaded to Meta'
    ...
    {modal && (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
        <div style={{
          background: 'var(--surface)', borderRadius: 16, padding: 36, width: 480, maxWidth: '90vw',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 18, color: modal.type === 'success' ? '#00e5a0' : '#ef4444' }}>
              {modal.title}
            </div>
          </div>
          ...
          {modal.details && (
            <div style={{ padding: '10px 14px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, marginBottom: 20 }}>
              ...
              <pre style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)', whiteSpace: 'pre-wrap', margin: 0 }}>{modal.details}</pre>
            </div>
          )}
