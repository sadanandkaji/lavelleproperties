...
export async function POST(req: Request) {
  try {
    ...
    const existing = await prisma.fullAmenity.findFirst({ where: { propertyId, normalized } });
    if (existing) return NextResponse.json(existing);

    const created = await prisma.fullAmenity.create({
      data: { propertyId, name: trimmed, normalized },
    });
    return NextResponse.json(created);
  } catch (error: any) {
    console.error("FULL_AMENITY_POST:", error);
...
export async function DELETE(req: Request) {
  try {
    ...
    await prisma.fullAmenity.delete({ where: { id } });
